...
void speech_commands_postprocess(const TfLiteTensor* score_tensor, std::vector<std::pair<uint8_t, float>> &class_results)
{
    ...
    //    class = class[::-1]
    //
    const float* data = reinterpret_cast<float*>(score_tensor->data.raw);

    TfLiteIntArray* output_dims = score_tensor->dims;
    // check output dimension
    assert(output_dims->size == 2);
    int batch = output_dims->data[0];
    int length = output_dims->data[1];

    auto unit = sizeof(float);
    int class_size = length;

    // TF/TFLite tensor format: NHWC
    auto bytesPerBatch = length * unit;
