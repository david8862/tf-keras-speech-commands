...
def main():
    ...
    if len(input_shape) == 3:
        ...
        # CNN model has 4 input dim
        batch, feature_num, feature_size, channel = input_shape
        assert channel == 1, 'invalid cnn channel number'
    else:
        raise ValueError('invalid input tensor shape')

    # to calculate FLOPs we need to use fixed input shape & batch size
    assert feature_num and feature_size, 'input shape should be specified'

    if not batch:
        # if dynamic batch, rebuild model with batch_size=1
        input_tensor = Input(shape=input_shape[1:], batch_size=1)
