...
# -*- coding: utf-8 -*-
"""
Analyze person's speech voice start/end time in wav file,
using VAD algorithm
...
def speech_duration_check(wav_file, vad_type):
    ...
    for speech_label in speech_labels:
        # check all the speech segment info to collect time
        if speech_label['speech_begin'] < speech_begin:
            speech_begin = speech_label['speech_begin']

        if speech_label['speech_end'] > speech_end:
            speech_end = speech_label['speech_end']

    if len(speech_labels) == 0:
        # no duration if not found speech
        speech_begin = 0.0
        speech_end = 0.0
...
def plot_speech_time(begin_array, end_array):
    # show hist for speech start & end time
    plt.figure(figsize=(15, 10))
    plt.subplot(1, 2, 1)
    plt.hist(begin_array, bins=40, alpha=0.7)
    plt.xlabel("start time(second)")
    plt.ylabel("number")
    plt.title("start time")

    plt.subplot(1, 2, 2)
    plt.hist(end_array, bins=40, alpha=0.7)
    plt.xlabel("end time(second)")
    plt.ylabel("number")
    plt.title("end time")

    plt.suptitle("hist for speech duration distribution of {} wav files".format(len(begin_array)))
    plt.show()



def main():
    parser = argparse.ArgumentParser(description='analyze speech voice duration in wav file, using VAD algorithm')
    ...
    if os.path.isfile(args.wav_path):
        speech_begin, speech_end = speech_duration_check(args.wav_path, args.vad_type)
        print('speech start at {}s, end at {}s'.format(speech_begin, speech_end))
    else:
        ...
        for wav_file in wav_files:
            # check duration for every wav file
            speech_begin, speech_end = speech_duration_check(wav_file, args.vad_type)
            speech_begin_list.append(speech_begin)
            speech_end_list.append(speech_end)
            pbar.update(1)

        pbar.close()
        speech_begin_array = np.array(speech_begin_list)
        speech_end_array = np.array(speech_end_list)

        # plot statistic hist for speech start/end time
        plot_speech_time(speech_begin_array, speech_end_array)
